...
1. 순서 매개변수 : 함수에서 사용하는 일반적인 매개변수로 작성순서대로 전달된다.
'''
#순서 매개변수
...
'''
2. 키워드 매개변수 : 순서와 상관없이 매개변수명에 따라 전달된다.
'''
#키워드 매개변수
...
'''
3. 기본 매개변수 : 인수가 전달되지 않는 경우 디폴트로 지정되는 값을 말한다.
'''
#기본 매개변수
...
#만약 Java에서의 호출이라면 첫 번째 호출에서 에러가 발생함
...
def famousMan(**man):
    ...
    for k in man: #딕셔너리는 key를 통해 value를 출력한다.
        print(k, "=>", man[k]) 
