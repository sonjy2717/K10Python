...
'''
print('=' * 5, "sorted()", '=' * 20)
numbers = (8, 7, 6, 8, 4, 9, 7, 5, 3, 2, 7, 4, 9, 8, 2, 6, 5) #튜플로 데이터 생성
...
list = [1,2,3,4]    # List 생성
it = iter(list)     # iterator 생성
while True: # 무한루프
    try:
...
iter()함수의 첫 번째 인자는 난수를 생성하는 람다식이 사용되었다.
'''
...
def number_generator1(): #제너레이터 함수 정의
    yield 0 #첫 번째 next()를 통해 0을 함수 밖으로 반환한다.
...
def number_generaotr2(n):
    i = 0
    while i < n:
        yield i #yield가 실행될때 현재 i값이 for문으로 전달되어 출력된다.
        i += 1
for i in number_generaotr2(5): #for문에서 호출할때 자동으로 next()가 실행된다.
    print(i) #실행결과 : 0, 1, 2, 3, 4
    
print('=' * 5, "제너레이터 함수3", '=' * 20)
def upper_generator(val):
    for i in val:
        yield i.upper() #리스트의 원소를 대문자로 변경한 후 함수밖으로 반환한다.
furits = ['apple', 'pear', 'grape', 'pineapple', 'orange']
for i in upper_generator(furits): #리스트의 인자만큼 반복 호출된다.
    print(i, end=' ') #실행결과 : 리스트의 문자열이 대문자로 출력된다.