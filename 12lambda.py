...
람다(lambda) 함수
    ...
    사용해서 간편하게 작성할 수 있고, 재사용되지 않는 1회성 함수를
    만들때 사용한다.
'''
...
'''
map() 함수
    : 전달된 데이터를 동일 함수에 반복적으로 적용시켜 주는 역할을
...
#list_data의 원소 개수만큼 람다식을 반복 호출하여 얻어진 결과를 리스트로 반환한다.
...
#50초과 & 1000미만인 경우에만 true를 반환하는 람다식을 통해 원소를 필터링 한다
...
'''
    ...
    정의 되었으므로 처음엔 1과 2가 인수로 주어져 더해지고,
    그 결과에 3을 더하고 다시 그 결과에 4를 더하는 구조로 이루어져 있다.
'''
