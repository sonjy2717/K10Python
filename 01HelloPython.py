...
print("x // y", x // y) #나누기. 소수부분을 제거하므로 항상 int형 결과를 반환한다.
print("x * y", x * y)
print("x ** y", x ** y) #거듭제곱. 2의 4제곱을 계산한다.
print("pow(x, y)", pow(x, y)) #함수를 통해 거듭제곱을 계산한다.
print("pow(x, y, z)", pow(x, y, z)) #2의 4제곱을 8로 나눈 나머지를 반환한다.
print("divmod(x, y)", divmod(x, y)) #x와 y를 나눈 몫과 나머지를 튜플로 반환한다.
print("divmod(z, x)", divmod(z, x)) #z와 x를 나눈 몫과 나머지를 튜플로 반환한다.
...
이와같이 더블 쿼테이션을 3개 작성한다.
"""
print(str)

head = "나는 헤더 "
bottom = " 나는 바텀"
print(head + bottom) #문자열 합치기
print(head * 3) #문자열 반복하기
print("=" * 30)

#문자열 슬라이싱 : 인덱스와 범위를 통해 접근할 수 있다.
engStr = "Hello Python Good"
print(engStr[0]) #0번 인덱스 : H
print(engStr[:3]) #0~2번 인덱스 : Hel
print(engStr[1:3]) #1~2번 인덱스 : el
print(engStr[1:]) #1~마지막까지 인덱스
...
'''
format()
    : 문자열에 format()함수를 사용하면 좀 더 발전된 스타일로
    문자열 포맷을 지정할 수 있다.
'''
#인덱스를 사용하는 방법
